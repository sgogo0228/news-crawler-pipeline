...
from my_fb_graphql_scraper.base.base_page import BasePage
from my_fb_graphql_scraper.pages.page_optional import PageOptional
from my_fb_graphql_scraper.utils.parser import RequestsParser
from my_fb_graphql_scraper.utils.locator import *
from my_fb_graphql_scraper.utils.utils import *
...
class FacebookGraphqlScraper(FacebookSettings):
    ...
    def get_user_posts(self, fb_username_or_userid: str, days_limit: int = 61, display_progress:bool=True) -> dict:
        ...
        if self.fb_account == None:
            res = self.requests_flow(doc_id = doc_id, fb_username_or_userid=user_id, days_limit=days_limit, profile_feed=profile_feed,orignial_name_or_id = fb_username_or_userid)
            return res
        # Scroll page
        counts_of_round = 0
    ...
    def requests_flow(self, doc_id:str, fb_username_or_userid:str, days_limit:int, profile_feed:list, orignial_name_or_id):
        """
        ...
        for i in range(loop_limit):
            ...
            self.requests_parser.parse_body(body_content=body_content,orignial_name_or_id=orignial_name_or_id)

            # Check progress
            next_page_status = get_next_page_status(body_content=body_content)
            before_time = str(self.requests_parser.creation_list[-1]-1)     #這邊不額外減1的話下個loop會抓到同一個時間的貼文
