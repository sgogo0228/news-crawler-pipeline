...
# 改了這個，原本如果貼文有引用，就只會抓到被引用文章的時間，導致時間記錄有誤
def find_creation(data, orignial_name_or_id):
    if isinstance(data, dict):
        ...
        if 'story' in data:
            ...
            if isinstance(data['story'], dict) and 'creation_time' in data['story'] and 'url' in data['story']:
                if orignial_name_or_id in data['story']['url']:
        ...
        for value in data.values():
            result = find_creation(value, orignial_name_or_id)
            if result:
                return result
        ...
        for item in data:
            result = find_creation(item, orignial_name_or_id)
            if result:
                return result
