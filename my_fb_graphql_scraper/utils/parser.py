...
class RequestsParser(object):
    ...
    def parse_body(self, body_content, orignial_name_or_id=None):
        for each_body in body_content:
            json_data = json.loads(each_body)
            self.res_new.append(json_data)
            # with open(r'C:\Users\sgogo\OneDrive\桌面\test.txt', 'a', encoding='utf-8') as f:
            #     f.write('----------------------------------\n')
            #     f.write(json_data.__str__() + '\n')
            try:
                ...
                if each_feedback:
                    ...
                    creation_time = find_creation(json_data, orignial_name_or_id)
